...
const PokemonName = require('./pokemon-name');
...
module.exports = Bookshelf.model('Pokemon', Bookshelf.Model.extend({
  ...
  pokemon_name() {
    return this.hasMany(PokemonName, 'pokemon_id');
  },
  ...
  capture_summary(query) {
    return Object.assign({
      ...
      nameList: this.related('pokemon_name').serialize(),
  ...
  CAPTURE_SUMMARY_RELATED: ['pokemon_name', 'boxes', 'game_family', 'game_family_dex_numbers'],
  RELATED: ['pokemon_name', 'boxes', 'game_family', 'game_family_dex_numbers', {
    locations(qb) {
      qb
